            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                if showAdherence && !adherencePoints.isEmpty {
                    chartSection(title: "Dose Adherence") {
                        Chart {
                            ForEach(adherencePoints) { pt in
                                BarMark(x: .value("Date", pt.date, unit: .day),
                                        y: .value("Scheduled", pt.total))
                                    .foregroundStyle(Color(hex: pt.colorHex).opacity(0.2))
                                BarMark(x: .value("Date", pt.date, unit: .day),
                                        y: .value("Taken", pt.taken))
                                    .foregroundStyle(Color(hex: pt.colorHex))
                            }
                            protoRuleMarks
                        }
                    }
